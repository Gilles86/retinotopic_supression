...
theta_ = get_alpha_vertex(np.concatenate([theta_l, theta_r]), mask, cmap='nipy_spectral', vmin=0.0, vmax=1.0, subject=pc_subject)


theta = get_alpha_vertex(pars['theta'].values, mask, cmap='hsv', vmin=-np.pi, vmax=np.pi, subject=pc_subject)
ecc = get_alpha_vertex(pars['ecc'].values, mask, cmap='viridis', vmin=0.0, vmax=4.0, subject=pc_subject)
