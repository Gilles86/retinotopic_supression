...
# encoder, INSTEAD of the m4 DoG-based version. Model 4 PRFs have a
# phantom σ-collapse failure mode that destroys voxel R² for ~all
# candidate V1 voxels (0/27 valid) and inflates high-r² selection
# across all ROIs; model 1 PRFs do not exhibit this.
#
# Phantom filter: voxels with R² >= 0.999 are dropped from the
# candidate pool (these are the phantom voxels).
#
# Same 8 shared parameters as the DoG sharedSigma fit:
...
# --- Logging. ---
LOGFILE="$HOME/logs/gauss_dyn_v3_target_sharedSigma_${SLURM_JOB_NAME:-target_shS}_${SLURM_JOB_ID}_${SLURM_ARRAY_TASK_ID:-0}.txt"
...
scontrol update jobid="${SLURM_JOB_ID}" \
    name="gauss_dyn_shS_sub-$(printf %02d "$subject")_${roi}" 2>/dev/null || true
...
echo "Running Gaussian (m1) + v3 + target + sharedSigma fit for sub-${subject}, roi=${roi}"
